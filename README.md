...
- 테스트 카테고리: 직장생활 / 집중력 / 연애 / AI·미래 / 성격 / 관계 / 라이프스타일 (전체 목록과 최신 개수는 `src/data/tests/index.ts`의 `allTests` 참고)
...
1. 테스트 콘텐츠와 정책 페이지(소개/개인정보처리방침/이용약관/문의)를 먼저 배포하고 실제 서비스로 안정화합니다.
...
public/         정적 자산 (favicon, robots.txt, manifest, ads.txt 등). og/ 폴더는 빌드 시 자동 생성됨
scripts/        빌드 후처리 스크립트 (정적 메타데이터/sitemap/OG 이미지 생성)
tests/          vitest 자동 테스트
workers/lib-root/  루트 도메인(lib.lc)용 별도 소형 Cloudflare Worker.
                   앱 본체와 무관하게 /ads.txt만 직접 서빙하고 나머지 요청은
                   전부 test.lib.lc로 301 리다이렉트한다 (AdSense는 루트 도메인
                   기준으로 ads.txt/사이트 검증을 하기 때문). `cd workers/lib-root
                   && npx wrangler deploy`로 별도 배포하며, 메인 앱의 Cloudflare
                   Git 연동 빌드와는 독립적이다.
```
